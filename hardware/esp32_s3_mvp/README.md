# 青韵智控 ESP32-S3 实测采集固件 v0.4.0

v0.4扩展只读 `info`：返回device_id、gas_enabled_mask、preflight_schema及i2c_present，逐一探测配置地址，不像原scan那样停止气路。用于[上位机启动自检](../../software/host_mvp/PREFLIGHT.md)，不代表I²C器件精度校准。

v0.3增加可选滚筒人工控制，协议`JSONL-v0.3`保留既有传感字段并追加电机字段。选型、GPIO、心跳、参数范围、独立安全链和上板验收见[DRUM-v0.1](../MOTOR_BASELINE.md)。`esp32s3_motor`为气路+滚筒构建；原`esp32s3`/`esp32s3_air`禁用滚筒。不把模拟、核心逻辑测试或编译结果当作实物验收。

硬件：**ESP32-S3-DevKitC-1 N8R8**。接线必须遵守 [HW-v0.2 引脚与电气规范](../PINOUT.md)。本目录是实测采集程序，不生成模拟读数；传感器未接、通信失败、称重未标定时对应字段为 `null` 并带诊断信息。

已完成三个构建环境的交叉编译、纯逻辑测试及上位机兼容性测试；**未烧录到实物板，未验证接线、测量精度和长期稳定性**。此前枚举到的 COM3/4/5/7 为蓝牙串口，恢复实物工作时须重新确认端口。
...
独立泵阀任务在接收到新命令后先关泵和两阀200ms，再开目标阀，400ms时才开泵，命令时长包含前400ms。使能断开后即丢弃当前动作，重新闭合不会恢复，须重新下发命令。USB拔出不会立即终止气路动作，仍按剩余时长结束；气路没有主机持续心跳控制。滚筒使用独立会话心跳，见DRUM-v0.1；没有自动采样/清洗循环。
...
当前不足：不具备SD离线缓存、传感器完整自动重连状态机、I²C总线时钟恢复、实时流量/阀位反馈、温漂补偿或自动试验循环。电机控制仅完成软件实现与构建，尚未上板。USB拥塞可能丢弃帧，主机通过seq识别；I²C故障下实际采样周期可能延长。设备校准与可靠性结论必须来自板上测试。
